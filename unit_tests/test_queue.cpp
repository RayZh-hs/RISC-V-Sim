...
void print_queue(const norb::FixedBufferedQueue<int, 5>& q, const std::string& title) {
    std::cout << "\n--- " << title << " ---\n";
    std::cout << "Contents: ";
...
int main() {
    norb::FixedBufferedQueue<int, 5> q;
    q.push(10);
    ...
    auto it = q.begin() + 1;
    std::cout << "Iterator 'it' points to: " << it.read() << std::endl;

    // Pop the front element (10)
    q.pop();
    print_queue(q, "After popping front (10)");
    norb::buffered_flush();
    print_queue(q, "After popping front (10) and flushing");
    ...
    std::cout << "After pop, 'it' still points to: " << it.read() << std::endl;
    ...
    q.push(50);
    print_queue(q, "After pushing 50");

    // 'it' is unaffected
    std::cout << "After more pushes, 'it' still points to: " << it.read() << std::endl;

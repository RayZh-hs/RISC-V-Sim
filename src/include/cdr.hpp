// cdr.hpp
// - declares the Common Dependency Resolver for rs, lsb and ba
...
namespace norb::riscv {
    ...
    // for templating classes the implementation is directly appended below

    template <int Capacity>
    void CommonDependencyResolver<Capacity>::listen_inbound() {
        auto &log = Logger::get();
        if (chan_inbound.has_data()) {
            ...
            log.as(LogLevel::INFO) << "[CDR] Writing new instruction=" << new_entry.repr();
            buffer.write_at(idx_new_entry, new_entry);
        }
    }

    template <int Capacity>
    void CommonDependencyResolver<Capacity>::listen_broadcast() {
        auto &log = Logger::get();
        if (cdb_ref->empty())
            return;
    ...
    template <int Capacity>
    std::optional<ResolvedInstructionEntry> CommonDependencyResolver<Capacity>::get_ready_entry() {
        auto &log = Logger::get();
        const auto ind =
    ...
    template <int Capacity>
    void CommonDependencyResolver<Capacity>::submit_executed_entry(rob_pointer_t rob_pointer, uint32_t value) {
        auto &log = Logger::get();

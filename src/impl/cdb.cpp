...
namespace norb::riscv {
    ...
    CommonDataBus::CommonDataBus() { norb::impl::BufferedManager::add(this); }

    CommonDataBus::~CommonDataBus() { norb::impl::BufferedManager::remove(this); }

    void CommonDataBus::broadcast(const BroadcastEntry& entry) { changes_queue_.push(entry); }

    bool CommonDataBus::empty() const { return broadcast_queue_.empty(); }

    BroadcastEntry CommonDataBus::read() const { return broadcast_queue_.front(); }

    std::optional<BroadcastEntry> CommonDataBus::read_as_optional() const {
        if (broadcast_queue_.empty()) return std::nullopt;
        return broadcast_queue_.front();
    }

    void CommonDataBus::flush() {
        if (not broadcast_queue_.empty()) broadcast_queue_.pop();
        // append changes

...
namespace norb::riscv {
    ...
    void LoadStoreBuffer::set_commit_bus(TemporaryBus<rob_pointer_t>& bus) { bus_con_commit.connect(bus); }
    ...
    void LoadStoreBuffer::on_commit() {
        ...
        const auto committed = bus_con_commit.read();
        if (committed.has_value()) {
            ...
            for (auto it = start; it != end; ++it) {
                log.as(LogLevel::DEBUG) << "Writing committed modification to memory: MemoryModification("
                                        << "address=" << it->second << ", value=" << modification_list[it->second]
                                        << ")";
